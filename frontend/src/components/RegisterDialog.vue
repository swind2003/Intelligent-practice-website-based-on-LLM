<template>
  ...
  <Transition name="dialog">
    <!-- 注册对话框遮罩层，点击空白处关闭对话框 -->
    <div class="register-overlay" v-if="visible" @click.self="closeDialog">
      <!-- 注册对话框主体 -->
    ...
    async handleRegister() {
      ...
      try {
        ...
        if (response.data.code === 200) {
          ...
          // 显示后端返回的错误信息
          this.showErrorMessage('注册失败', response.data.message);
        }
      } catch (error) {
        console.error('注册出错:', error);
        // 网络错误或其他异常
        this.showErrorMessage('注册失败', '未知错误');
      }
    }
  },
